...
def FilterReads(inbam, outbam, readfilter='singleend', MQ=2, minIdentity=0.95, NM=-1, refname_to_chr=r_to_c_hg38):
    ...
    if readfilter == 'pairedend' or readfilter == 'pseudosingleend':
        for read, mate in read_pair_generator(infile):
            if read.reference_name not in refname_to_chr or mate.reference_name not in refname_to_chr:
                continue
            if (read.is_unmapped or mate.is_unmapped):
                continue
            if (read.mapping_quality < MQ or mate.mapping_quality < MQ):
                continue
            if (float(read.get_tag("XI")) < minIdentity or float(mate.get_tag("XI")) < minIdentity):
                continue
            if ((NM > -1 and int(read.get_tag("NM")) > NM) or (NM > -1 and int(mate.get_tag("NM")) > NM)):
                continue
...
def plot_filteredreads(bamfiles, labels, output):
    ...
    fig, ax = plt.subplots()
    ax.bar(labels, mapped_reads, 0.35, label='mapped', edgecolor='black', color="#999999")
    ax.bar(labels, unmapped_reads, 0.35, bottom=mapped_reads, label='unmapped',
                 edgecolor='black', color="#E69F00")
    ax.bar(labels, ambiguous_reads, 0.35, bottom=np.array(mapped_reads) + np.array(unmapped_reads),
                 label='ambiguous', edgecolor='black', color="#56B4E9")
